...
static __inline void _fillbfr(int size)
{
        memmove(_buffer,in_file,size);
        in_file+=size;
		bytes_read+=size;
        _bptr=0;
}

__inline unsigned int _getbits(int n)
{
...
int overflow;
        ...
        memmove(&buffer[append],in_file,advance);
        in_file+=advance;
		bytes_read+=advance;
 ...
 */
__inline unsigned int viewbits(int n)
{
unsigned int pos,ret_value;
